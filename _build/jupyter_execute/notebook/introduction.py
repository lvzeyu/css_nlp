...
# 
# 2014年頃、この分野において、<u>**非線型ニューラルネットワーク**</u>が導入され、多くのタスクにより高い性能を達成できました。さらに、これに基づいて、より先進的なモデリング方法の開発も進めました。特に、<u>**再帰的ニューラルネットワーク(RNN)**</u>に基づく方法は言語の時系列性質も学習できるになって、様々なタスクにおいて精度向上に大きく貢献しました。
# 
# 2018年にGoogleが発表した<u>BERT</u>というモデルでは、大規模なテキストで一般的な言語パターンや文脈を学習します。その後、この<u>**事前学習済みモデル**</u>を特定のNLPタスクに<u>**ファインチューニング**</u>することで、少ないラベル付きデータで高性能を発揮することができます。
# 
# それだけでも驚きでしたが、一般人の中でも話題になる「ChatGPT」をはじめとする生成AIの<u>**大規模な言語モデル(LLM:Large Language Model)**</u>の進展により、質問への回答、文章の要約や翻訳、ソフトウエアのプログラミングなど、言語に関わるさまざまなタスクができるようになりました。
# 
# <u>**高機能化のカギは、深層学習技術の発展があります。**</u>深層学習を用いた自然言語処理には、あらかじめ用意した膨大な文章を使って、「言語モデル」と呼ばれるシステムを学習させる方法があります。
...
# 
# 本講義の目的は、<u>**計算社会科学に多く応用された自然言語処理技術を理解し、実問題に適用するための基礎力を身につけることです。**</u>
# 
# その目的を達成するために、<u>**自然言語処理と深層学習の基礎、重要な概念と主な手法(モデル)**</u>を学習する。さらに、Python用いて、自然言語処理によく用いられるライブラリとツールを学習しつつ、<u>**自然言語処理技術を応用するスキル**</u>を修得する。
# 
...
# 
# 
# - 1910年代からのテキストデータで単語分散表現を学習し、単語分散表現で男性と女性はそれぞれどのような単語と関連していることを検証することで、ジェンダーのステレオタイプの実態と変化を定量的に分析する {cite}`Garg2018` 
# 
# ![](./Figure/word2vec_gender.jpeg)
# 
# - 単語分散表現の類似性と単語類推特性で、単語分散表現の計算を通じて、文化概念の潜在的意味と関係を定量的に測定できました{cite}`　Kozlowski2019`
# 
# ![](./Figure/city.png)
# 
...
#     - Self-attention
#     - Transformerの構成要素
# 13. BERTによるテキスト分類の実装(1) &#x1F4BB;
#     - 事前学習済みモデルと転移学習
#     - HuggingFace
#     - GPUの設定
# 14. BERTによるテキスト分類の実装(2) &#x1F4BB;
# 15. 大規模言語モデル &#x1F4C4;
# 
