...
# 
# **<u>教師あり学習（Supervised Learning）では、入力データ（特徴量）とそれに対応する正解ラベル（目標値）のペアを使用してモデルを訓練します。**</u>
# 
# モデルは、**<u>入力データと正解ラベルの間の関係やパターンを学習し、未知の入力データに対して正しい予測や分類を行うことが期待されます。**</u>テキスト分類、構文解析、機械通訳などの様々な自然言語処理タスクは、教師あり学習の問題として定式化できます。
# 
# 教師データで学習した入出力の関係性がラベルを持たない未知の入力データにも使えるような関係性である必要があります。このように未知のデータにも対応できる性質を **<u>汎用性**</u> と言います。
# 
...
# - **ミニバッチ学習**: バッチ学習とオンライン学習の中間のような学習手法であり、**<u>データをミニバッチという小さなグループに分割してモデルを学習します。**</u>バッチ学習は、計算効率とメモリの使用のバランスが良いこと、および学習の収束速度が適切であるため、一般的に最も使用される方法です。
# 
# 
...
# ## 過学習
# 
# **<u>過学習(Overfitting)**</u>とは、データの傾向に沿うようにモデルを学習させた結果、学習時のデータに対してはよい精度を出すが、未知データに対しては同様の精度を出せないモデルが構築されてしまうことです。
# 
# ![](./Figure/overfitting.jpeg)
# 
# 過学習を防ぐための一つの方法としては、交差検証法を使うことです。
# 
# **<u>交差検証とは、1つのデータを訓練データと検証データに分けるときに複数の分け方をして平均をとるという方法です。**</u>　データの分け方を複数作ることでリスクを分散し、訓練データと検証データの傾向の違いにより生じる過学習を最小化します。
# 
# 最もよく使われるK-交差検証では、
# 
# - 全体データをK個にデータを分割します。
...
# ## 機械学習のハイパーパラメータ
# 
# **<u>機械学習のハイパーパラメータは、学習アルゴリズムの動作を制御するためのパラメータです。**</u>　
# 
...
# ### エポック(epoch)数
# 
# 機械学習・深層学習ようにパラメータの数が多いものになると、訓練データを何回も繰り返する必要があります。**<u>エポック数とは、学習において、データセットを何週繰り返してパラメータを調整するかを表す数を指します。1エポックは、トレーニングデータセット全体が一度、モデルを通過することを意味します。**</u>
# 
...
# ### 学習率(Learning Rate)
# 
# **<u>学習率とは、機械学習の最適化において、重みパラメータを一度にどの程度変化させるかを表すハイパーパラメータのことです。**</u>
# 
